(() => {
    ...
    function formFieldsInit(options = {
        ...
        document.body.addEventListener("focusout", (function(e) {
            ...
            if (targetElement.tagName === "INPUT" || targetElement.tagName === "TEXTAREA") {
                ...
                targetElement.hasAttribute("data-validate") ? formValidate.validateInput(targetElement, options.inputFileValidateProps) : null;
            }
        }));
    ...
    let formValidate = {
        getErrors(form, requreProps) {
            ...
            if (formRequiredItems.length) formRequiredItems.forEach((formRequiredItem => {
                if ((formRequiredItem.offsetParent !== null || formRequiredItem.tagName === "SELECT") && !formRequiredItem.disabled) error += this.validateInput(formRequiredItem, requreProps);
            }));
            return error;
        },
        validateInput(formRequiredItem, requreProps) {
            ...
            if (dataset.required === "email") {
            ...
            } else if (type === "checkbox") checkAndHandleError(!checked); else if (type === "tel") {
                ...
                checkAndHandleError(validateMessage, validateMessage);
            } else checkAndHandleError(!value.trim(), "Field is required");
            return error;
        },
        addError(formRequiredItem, validateMessage) {
            ...
            if (formRequiredItem.dataset.error || validateMessage) formRequiredItem.parentElement.insertAdjacentHTML("beforeend", `<div class="form__error">${formRequiredItem.dataset.error ? formRequiredItem.dataset.error : validateMessage}</div>`);
        },
        ...
        fileTest(formRequiredItem, requreProps) {
            ...
            const allowedTypes = formRequiredItem.getAttribute("data-types") ? formRequiredItem.getAttribute("data-types").split(",").map((type => type.trim())) : requreProps?.allowedTypes || [];
            if (!formRequiredItem.files || formRequiredItem.files.length === 0) return "No file selected.";
            for (const file of formRequiredItem.files) {
                ...
                if (maxSize && file.size > maxSize * 1024 * 1024) return `Max size is ${maxSize} MB.`;
            }
            return null;
        },
        phoneNumberTest(formRequiredItem) {
            ...
            if (!formRequiredItem.value.trim()) return "Enter your phone number";
            if (!/^[+\d\s\-\(\)]+$/.test(formRequiredItem.value)) return 'Only: "digits, spaces, -, (, ), +" ';
            const cleaned = formRequiredItem.value.replace(/[^\d]/g, "");
            if (cleaned.length < minValue) return `Min ${minValue} digits.`;
            if (cleaned.length > maxValue) return `Max ${maxValue} digits.`;
            return null;
        }
    };
    function formSubmit(props) {
        ...
        async function handleFormSubmit(form, e) {
            const error = !form.hasAttribute("data-no-validate") ? formValidate.getErrors(form, inputFileValidateProps) : 0;
            if (error > 0) {
                e.preventDefault();
                scrollToError(form);
                return;
        ...
        async function handleAjaxSubmit(form) {
            ...
            try {
                if (isTelegram) await sendToTelegram(form); else await sendToServer(form);
            } catch (error) {
        ...
        async function sendToServer(form) {
            ...
            } else throw new Error("Error while sending form data to server");
        }
        async function sendToTelegram(form) {
            ...
            formData.forEach(((value, key) => {
                if (value instanceof File && value.name) {
                    ...
                    mediaData.push({
                        type: "document",
                        media: `attach://file${fileIndex}`
                    });
                    message += `<b>${key}:</b> ${value.name}\n`;
                    fileIndex++;
                } else if (!(value instanceof File)) message += `<b>${key}:</b> ${value}\n`;
            }));
        ...
        async function sendFilesToTelegram(URL_API, fileData) {
            ...
            if (!response.ok) throw new Error("File send error");
        }
    ...
    if (fileInpLabel && inpFile) inpFile.addEventListener("change", (function() {
        if (inpFile.files.length === 1) {
            fileInpLabel.textContent = inpFile.files[0].name;
            fileInpLabel.style.color = "#000";
        } else if (inpFile.files.length > 1) {
            fileInpLabel.textContent = `Choosed ${inpFile.files.length} files`;
            fileInpLabel.style.color = "#000";
        } else fileInpLabel.textContent = "Choose file";
    }));
    addLoadedClass();
    menuInit();
    spollers();
